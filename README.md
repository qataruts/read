# اِقْرَأْ — تعليم القراءة العربية لطفل السادسة

> **اِقْرَأْ** — يعلّم طفلك قراءة العربية، من الحرف الأول إلى القرآن.
> (اسمُ المشروع في سجلاته التاريخية «المُعلِّم»؛ العلامةُ «اِقْرَأْ» منذ ٨ أغسطس ٢٠٢٦ — `docs/REVIEW_BRAND.md`.)
...
## الصفحة التعريفية (للمعلمين والمدارس)

`‎/welcome/` — صفحةٌ تعريفية بالتطبيق على شجرة النشر نفسها
(`http://127.0.0.1:8000/welcome/` محلياً): ما «اِقْرَأْ» ولمن، والأسس الخمسة، ورحلة
التعلم بلقطاتٍ حقيقية من شاشاته، وكيف يُثبَّت على الجهاز (PWA)، و**دليل المعلم**
(`‎/welcome/#guide`) بخريطة التسلسل كاملةً ومسوّغات القرارات المنهجية وحدود النطاق.

وهي **خارج التطبيق وخارج قشرة عامل الخدمة** عمداً: لا تُخزَّن، ولا تسجّل عاملاً، ولا
تصل بيان التطبيق، ولا تحمل سطر جافاسكربت واحداً — فلا أثر لها على PWA المثبَّتة.
وزرّ «ابدأ الآن» ينقل إلى التطبيق (`../`).

```bash
python3 tools/make_welcome_shots.py          # يولّد لقطات الرحلة من شاشات التطبيق نفسِها
python3 tools/make_welcome_shots.py --check  # تحقّق منها بلا توليد
```
...
| `app/welcome/` | الصفحة التعريفية ودليل المعلم (خارج قشرة عامل الخدمة) ولقطاتها المولَّدة |
...
node tools/test_welcome.mjs          # الصفحة التعريفية: خارج القشرة، وأرقامها محسوبة من المنهج
