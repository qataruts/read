...
# حدُّ الرتابة اللفظية (مأخذ المدير، ١٢ أغسطس ٢٠٢٦): عبارةٌ من `PHRASE_LEN` كلمات
# لا تتنقّل في أكثر من `PHRASE_SPREAD` قصص. انظر علّته في `self_test`.
PHRASE_LEN = 3
PHRASE_SPREAD = 3
...
# ————— جردُ أصناف كلمة الأساس: أيُّ دورٍ يقبله أيُّ صنف —————
#
# **العلّة، وهي ثالثةُ الصنف** (تصحيح المالك، ١٢ أغسطس ٢٠٢٦): صار في المولّد
# «الزَّيْد» — والعلَمُ لا تدخله «ال». وقبلها منعُ التنوين على لفظ الجلالة، وقبلهما
# التطويل. **وثلاثةٌ من صنفٍ واحد تعني أنّ الحالة تُعالَج والبابُ مفتوح**: كلُّ مرّةٍ
# سُدَّت ثغرةٌ بعينها. فيُجرَد الصنفُ كلُّه مرّةً واحدة، ويُقفَل الدورُ لا الحالة:
#
#   • **عَلَم** — لا «ال» ولا حركةَ إعرابٍ مجرّدة: صورُه ثلاثٌ لا رابعَ لها —
#     مجرَّدٌ («أَمَامَ زَيْدْ» · «مَعَ زَيْدْ» · «جَدَّةُ حَسَنْ»)، و`~N` مرفوعاً في
#     الوسط («زَيْدٌ يَبْكِي»)، و`~A` منصوباً موقوفاً عليه («يُسَاعِدُ زَيْدَا»).
#   • **لفظُ الجلالة** — معرفةٌ بذاته: لا «ال» ولا تنوين، و`~u` وحدَه («اللَّٰهُ»).
#   • **مضافٌ إلى ياء المتكلم** — معرفةٌ بالإضافة: «الْأَبِيُ» و«أَبِيٌ» ليستا عربيتين،
#     فلا «ال» ولا تنوين (وهي قاعدةُ `DRESSLESS` نفسُها في سلّم الجمل، مطبَّقةً هنا).
#   • **نكرة** — وهو الأصل: تقبل الأدوار كلَّها.
#
# **والبابُ يُقفَل بالجرد لا بالقائمة**: يُثبِت `self_test` أنّ **كلَّ أصلٍ ينتهي بياء
# المتكلم مصنَّفٌ** — مضافاً أو مستثنىً بعلّته (فعلٌ مضارع، أو نسبةٌ، أو منقوصٌ آخرُه
# ياءُ مدّ) — فأصلٌ جديد يُعلَن غداً بياءٍ ولا يُصنَّف **يُسقِط الفحصَ يومَ يُعلَن**.

PROPER_NOUNS = {"زَيْدْ", "سَامِي", "حَسَنْ", "جُحَا"}      # أعلامُ مادّتنا كلُّها

POSSESSIVE = {          # مضافٌ إلى ياء المتكلم (معرفةٌ بالإضافة)
    "أَبِي", "أَخِي", "أُخْتِي", "أُمِّي", "جَدِّي", "خَالِي", "زَمِيلِي", "صَدِيقِي",
    "أَسْنَانِي", "أَنْفِي", "أُذُنِي", "بَطْنِي", "بِنْصِرِي", "جَبِينِي", "جِلْدِي",
    "حَاجِبِي", "خَدِي", "دِمَاغِي", "ذِرَاعِي", "رَأْسِي", "رِجْلِي", "شَعْرِي",
    "صَدْرِي", "صَوْتِي", "ضِرْسِي", "ظَهْرِي", "ظُفْرِي", "فَمِي", "قَدَمِي", "قَلْبِي",
    "كَتِفِي", "لِسَانِي", "مِرْفَقِي", "مِعْصَمِي", "نَفَسِي", "وَجْهِي", "تُحِبُّنِي",
}

# مستثنىً بعلّته: ياؤه ليست ياءَ المتكلم — فيبقى نكرةً تقبل الأدوار كلَّها
YA_EXEMPT = {
    # فعلٌ مضارع آخرُه ياء (دورُه `~v` لا غير)
    "تَجْرِي", "تَشْتَرِي", "تَمْشِي", "تُعْطِي", "يَبْكِي", "يَبْنِي", "يَجْرِي",
    "يَشْتَرِي", "يَمْشِي", "يُدَاوِي", "يُعْطِي",
    # نسبةٌ: «اللونُ الْبُرْتُقَالِي» عربيةٌ صحيحة، فلا تُمنَع من «ال»
    "بَنَفْسَجِي", "بُرْتُقَالِي", "رَمَادِي", "وَرْدِي",
    # منقوصٌ آخرُه ياءُ مدّ — اسمٌ نكرةٌ يُعرَّف («الْوَادِي» · «الرَّاعِي»)
    "رَاعِي", "وَادِي", "غَالِي",
    # اسمُ عَلَم (مُصنَّفٌ في `PROPER_NOUNS`)، وحرفُ جرّ
    "سَامِي", "فِي",
}

# الأدوارُ الممنوعة على كل صنف — والمنعُ **بنيويّ**: يوقف البناء ولا يمرّ
DEFINITE = frozenset("niap")        # أدوارُ «ال» (نظيرُ DEFINITE_ROLES في ٩أ)
TANWIN = frozenset("NA")
CASE_BARE = frozenset("ukf")        # حركةُ إعرابٍ بلا «ال» ولا تنوين
FORBIDDEN = {
    "علم": DEFINITE | CASE_BARE | frozenset("m"),
    "لفظ الجلالة": DEFINITE | TANWIN | frozenset("m"),
    "مضاف إلى ياء المتكلم": DEFINITE | TANWIN | CASE_BARE | frozenset("m"),
}


WHY = {
    "علم": "العَلَمُ لا تدخله «ال» ولا يُجرّ بحركةٍ مجرّدة: مجرَّدٌ («أَمَامَ زَيْدْ») "
           "أو `~N` («زَيْدٌ يَبْكِي») أو `~A` («يُسَاعِدُ زَيْدَا»)",
    "لفظ الجلالة": "معرفةٌ بذاته: لا «ال» ولا تنوين — و`~u` وحدَه («اللَّٰهُ»)",
    "مضاف إلى ياء المتكلم": "معرفةٌ بالإضافة: «الْأَبِيُ» و«أَبِيٌ» ليستا عربيتين",
}


def word_class(base: str) -> str:
    """صنفُ كلمة الأساس — والأصلُ «نكرة»، وما سواه معلَنٌ في جرده أعلاه."""
    if base == LAFZ_JALALA:
        return "لفظ الجلالة"
    if base in PROPER_NOUNS:
        return "علم"
    if base in POSSESSIVE:
        return "مضاف إلى ياء المتكلم"
    return "نكرة"
...
SHELF_SUPPORT = [
    ...
    # ————— زيادةُ المستوى السادس —————
    "يَصْحُو", "يُمْسِكْ", "يَسْمَعْ", "يَقْطَعْ", "يَخْرُجْ",   # عقدةُ الأسد والفأر
    "تَصِيحْ", "جَائِعَةْ", "حَزِينَةْ",                          # مؤنّثُ حالِ الحمامة وجماعتها
    "ثُمَّ",                                                      # أداةُ عطفٍ **منفصلة** — بها تُبنى الجملةُ الطويلة (والملتصقُ ممنوع)
    "يُحَاوِلْ", "يَفُوزْ", "مَرَّةْ",                             # سباقُ زيد: محاولةٌ ففوز
    "نَائِمْ", "ضَعِيفْ",                                         # حالٌ يُروى لا حكمٌ يُقال
    "يَأْخُذْ",           # «زَيْدٌ يَأْخُذُ الْكَأْسَ مِنَ الْمُعَلِّمْ» — انظر علّتَها في صفحتها
]
...
SPECS = [
    ...
    # ————— رفُّ المكتبة · المستوى ٦: أربع عشرة صفحة (٤–٨ كلمات) —————
    #
    # **وتوجيهُ المدير في التنويع اللفظيّ** (١٢ أغسطس ٢٠٢٦): «فَوْقَ الشَّجَرَةِ
    # الْعَالِيَةْ» تكرّرت في أربعٍ من ستّ — والتكرارُ نافعٌ للمبتدئ، لكنّ الرتابةَ
    # مأخذ. فمشاهدُ هذه الثلاث **من مواضعَ أخرى**: كهفٌ وصخرةٌ وتلٌّ وغصنٌ ونهرٌ
    # وحقلٌ وملعب — والميزانيةُ تسمح (٧٩١ مفردة)، فلا عذرَ في الرتابة.
    {
        ...
        "cover": {"hero": "🦁", "mood": "deep"},
        "title": "أَسَدْ~n +فَأْرْ~p",
        "pages": [
            ("فِي غَابَةْ~i كَبِيرَةْ~i أَسَدْ~N قَوِيّْ", "🌳"),
            ("أَسَدْ~n قَوِيّْ~n يَنَامْ~v فِي كَهْفْ~i كَبِيرْ~p", "😴"),
            ("فَأْرْ~N صَغِيرْ~N يَجْرِي فَوْقَ صَخْرَةْ~p", "🐭"),
            ("فَأْرْ~n يَقْفِزْ~v فَوْقَ أَسَدْ~i نَائِمْ~p", "🦘"),
            ("أَسَدْ~n يَصْحُو~v ثُمَّ يُمْسِكْ~v فَأْرْ~p", "😾"),
            ("فَأْرْ~n ضَعِيفْ~n يَصِيحْ~v فِي كَهْفْ~p", "😰"),
            ("أَسَدْ~n طَيِّبْ~n يَتْرُكْ~v فَأْرْ~p", "💚"),
            ("فَأْرْ~n سَعِيدْ~n يَجْرِي خَلْفَ تَلّْ~p", "🏃"),
            ("صَيَّادْ~N يَضَعْ~v حَبْلْ~a طَوِيلْ~a خَلْفَ تَلّْ~p", "🧵"),
            ("أَسَدْ~n قَوِيّْ~n يَسْقُطْ~v فِي حَبْلْ~i طَوِيلْ~p", "🪤"),
            ("أَسَدْ~n حَزِينْ~n يَصِيحْ~v فِي غَابَةْ~p", "🗣️"),
            ("فَأْرْ~n صَغِيرْ~n يَسْمَعْ~v صَوْتْ~a كَبِيرْ~p", "👂"),
            ("فَأْرْ~n يَجْرِي ثُمَّ يَقْطَعْ~v حَبْلْ~p", "✂️"),
            ("أَسَدْ~n كَبِيرْ~n يَشْكُرْ~v فَأْرْ~a صَغِيرْ~p", "🤝"),
        ],
        "asks": [
            # (ولا تصلح «صَخْرَةْ» جواباً: معلَنةٌ `pictured: false` — لا صورةَ صادقة لها)
            ("أَيْنَ يَنَامْ~v أَسَدْ~p", "كَهْفْ", ["سَرِيرْ", "قَارِبْ"]),
            ("مَنْ يَضَعْ~v حَبْلْ~p", "صَيَّادْ", ["مُعَلِّمْ", "خَبَّازْ"]),
            ("مَاذَا يَقْطَعْ~v فَأْرْ~p", "حَبْلْ", ["خَيْطْ", "وَرَقَةْ"]),
        ],
    },
    {
        ...
        "pages": [
            ...
            ("زَمِيلْ~N سَرِيعْ~N يَجْرِي أَمَامَ زَيْدْ", "🏃"),
            ("زَمِيلْ~n سَرِيعْ~n يَحْمِلْ~v كَأْسْ~a جَدِيدْ~p", "🏆"),
            ("زَيْدْ~N حَزِينْ~N يَجْلِسْ~v فَوْقَ عُشْبْ~p", "😔"),
            ("مُعَلِّمْ~N طَيِّبْ~N يَجْلِسْ~v مَعَ زَيْدْ", "🧑‍🏫"),
            ("زَيْدْ~N يَسْمَعْ~v صَوْتْ~a طَيِّبْ~p", "👂"),
            ("زَيْدْ~N يُحَاوِلْ~v مَرَّةْ~a جَدِيدَةْ~p", "💪"),
            ("زَيْدْ~N يَجْرِي خَلْفَ زَمِيلْ~i سَرِيعْ~p", "🏃"),
            ("زَيْدْ~N يَجْرِي فَوْقَ طَرِيقْ~i طَوِيلْ~p", "🛤️"),
            ("زَيْدْ~N سَرِيعْ~N يَجْرِي أَمَامَ زَمِيلْ~p", "⚡"),
            ("زَيْدْ~N يَفُوزْ~v فِي سِبَاقْ~i كَبِيرْ~p", "🎉"),
            # **الرتبةُ الطبيعية تُشترى بتبديل الجملة لا بقلبها** (تصحيح المالك):
            # كانت «الْمُعَلِّمُ يُعْطِي زَيْدَ الْكَأْسْ» فدخلت «ال» على العَلَم؛ فقُلبت
            # الرتبةُ («يُعْطِي الْكَأْسَ الْجَدِيدَ زَيْدَا») فصحّت نحواً وثقُلت ذوقاً —
            # **والمنظومةُ صفرُ تنوينِ نصبٍ وسط الجملة**، فالقلبُ كان الطريقَ الوحيد في
            # نظام الأدوار. والمخرجُ أن تُبدَّل الجملةُ نفسُها: يأخذ الطفلُ الكأسَ من
            # معلّمه — طبيعيةٌ لطفل، ولا عَلَمَ في موضع نصبٍ أصلاً.
            ("زَيْدْ~N يَأْخُذْ~v كَأْسْ~a مِنَ مُعَلِّمْ~p", "🏆"),
            ("زَيْدْ~N سَعِيدْ~N يُعْطِي زَمِيلْ~a كَأْسْ~p", "🤝"),
            ("زَيْدْ~N يَجْرِي مَعَ زَمِيلْ~i سَعِيدْ~p", "😀"),
        ],
        "asks": [
            ("مَاذَا يَحْمِلْ~v زَمِيلْ~p", "كَأْسْ", ["دُمْيَةْ", "طَبْلْ"]),
            ("مَنْ يَجْلِسْ~v مَعَ زَيْدْ", "مُعَلِّمْ", ["طَبِيبْ", "جَارْ"]),
            ("أَيْنَ يَجْرِي زَيْدْ", "مَلْعَبْ", ["مَتْحَفْ", "مَخْبَزْ"]),
        ],
    },
    {
        ...
        "cover": {"hero": "🕊️", "mood": "teal"},
        "title": "حَمَامَةْ~n +جَمَاعَةْ~p",
        "pages": [
            ("حَمَامَةْ~N جَائِعَةْ~N تَطِيرْ~v فَوْقَ حَقْلْ~p", "🕊️"),
            ("جَمَاعَةْ~n كَبِيرَةْ~n تَطِيرْ~v خَلْفَ حَمَامَةْ~p", "🐦"),
            ("صَيَّادْ~N يَضَعْ~v ذُرَةْ~a صَغِيرَةْ~a تَحْتَ غُصْنْ~p", "🌽"),
            ("صَيَّادْ~n يَضَعْ~v حَبْلْ~a رَفِيعْ~a فَوْقَ ذُرَةْ~p", "🪢"),
            ("حَمَامَةْ~n جَائِعَةْ~n تَنْزِلْ~v فَوْقَ ذُرَةْ~p", "⬇️"),
            ("جَمَاعَةْ~n كَبِيرَةْ~n تَأْكُلْ~v ذُرَةْ~a صَغِيرَةْ~p", "🌾"),
            ("حَبْلْ~n رَفِيعْ~n يُمْسِكْ~v جَمَاعَةْ~p", "🪤"),
            ("جَمَاعَةْ~n حَزِينَةْ~n تَصِيحْ~v تَحْتَ غُصْنْ~p", "😰"),
            ("حَمَامَةْ~n صَبُورَةْ~n تَنْظُرْ~v فَوْقَ جَمَاعَةْ~p", "👀"),
            ("جَمَاعَةْ~n كَبِيرَةْ~n تَطِيرْ~v مَعَ حَبْلْ~p", "🕊️"),
            ("حَبْلْ~n رَفِيعْ~n يَخْرُجْ~v مِنَ حَقْلْ~p", "🌤️"),
            ("فَأْرْ~N طَيِّبْ~N يَخْرُجْ~v مِنَ تَلّْ~p", "🐭"),
            ("فَأْرْ~n صَغِيرْ~n يَقْطَعْ~v حَبْلْ~a رَفِيعْ~p", "✂️"),
            ("جَمَاعَةْ~n سَعِيدَةْ~n تَطِيرْ~v فَوْقَ نَهْرْ~p", "😀"),
        ],
        "asks": [
            ("مَاذَا يَضَعْ~v صَيَّادْ~p", "ذُرَةْ", ["جُبْنْ", "عَسَلْ"]),
            ("مَاذَا تَأْكُلْ~v جَمَاعَةْ~p", "ذُرَةْ", ["مَوْزْ", "سَمَكْ"]),
            ("مَنْ يَقْطَعْ~v حَبْلْ~p", "فَأْرْ", ["قِرْدْ", "بُومَةْ"]),
        ],
    },
    ...
    {
        # **وجهُها الكعبةُ لا الفيل** (أمر المالك، ١٢ أغسطس ٢٠٢٦): بطلةُ الخبر
        # الكعبةُ المحفوظة لا الفيلُ الممنوع — وهو ما تنتهي إليه القصةُ نصّاً
        # («اللَّٰهُ يَحْفَظُ الْكَعْبَةْ») وما تبدأ به. فالوجهُ يصدُق المعنى لا الحادثة،
        # ومنه يُشتقّ الغلافُ فلا يفترقان. و🕋 من رموز صفحاتها (شرطُ صدق الصورة).
...
def render_token(token: str, bases: dict, *, title: bool = False) -> str:
    ...
    # **الصنفُ يحكم الدور قبل أن يُشتقّ شيء** (جرد الأصناف أعلاه): فما امتنع نحواً
    # يوقف البناءَ ولا يمرّ — لا حالةً بعينها تُرقَّع بعد أن تظهر على الشاشة.
    kind = word_class(base)
    if role and role in FORBIDDEN.get(kind, frozenset()):
        raise ValueError(f"«{base}» {kind}، والدورُ «~{role}» ممتنعٌ عليه — "
                         + WHY.get(kind, ""))
...
def self_test(data: dict) -> int:
    ...
    # ————— جردُ أصناف كلمة الأساس: البابُ يُقفَل لا الحالة —————
    #
    # **مُجرَّبٌ سالباً على كل صنف**: لا يكفي أن تمرّ المادةُ اليوم، بل أن **يُردّ**
    # ما امتنع نحواً غداً — فهذا هو الفرقُ بين إقفال الباب وترقيع الحالة.
    def refused(token):
        try:
            render_token(token, bases)
            return ""
        except ValueError as e:
            return str(e)

    ok(all(refused(f"زَيْدْ~{r}") for r in "niapukfm"),
       "العَلَمُ يُردّ عليه دورُ التعريف وحركةُ الإعراب المجرّدة (زَيْدْ · سَامِي · حَسَنْ · جُحَا)")
    ok(not refused("زَيْدْ~N") and not refused("زَيْدْ~A") and not refused("زَيْدْ"),
       "— وتبقى صورُه الثلاث: مجرَّدٌ و`~N` و`~A` («أَمَامَ زَيْدْ» · «زَيْدٌ يَبْكِي» · «زَيْدَا»)")
    ok(all(refused(f"{LAFZ_JALALA}~{r}") for r in "niapNAm") and not refused(f"{LAFZ_JALALA}~u"),
       "ولفظُ الجلالة يُردّ عليه التعريفُ والتنوين معاً، ويبقى `~u` («اللَّٰهُ»)")
    ok(all(refused(f"أُمِّي~{r}") for r in "niapNAukfm") and not refused("أُمِّي"),
       "والمضافُ إلى ياء المتكلم مجرَّدٌ لا غير («الْأَبِيُ» و«أَبِيٌ» ليستا عربيتين)")
    ok(not refused("مِفْتَاحْ~n") and not refused("مِفْتَاحْ~N"),
       "والنكرةُ تقبل الأدوار كلَّها — وهي الأصل، وما سواه معلَنٌ في الجرد")

    # **والبابُ يُقفَل بالجرد**: كلُّ أصلٍ ينتهي بياء المتكلم مصنَّفٌ — مضافاً أو
    # مستثنىً بعلّته. فأصلٌ جديد يُعلَن غداً بياءٍ ولا يُصنَّف يُسقِط هذا الفحصَ يومَ يُعلَن.
    ya = {t for t in bases if t.endswith("ِي")}
    unclassed = sorted(ya - POSSESSIVE - YA_EXEMPT)
    ok(not unclassed, f"وكلُّ أصلٍ آخرُه ياءُ المتكلم مصنَّفٌ ({len(ya)} أصلاً: "
       f"{len(ya & POSSESSIVE)} مضافاً و{len(ya & YA_EXEMPT)} مستثنىً بعلّته)"
       + (f" — بلا صنف: {unclassed}" if unclassed else ""))
    ok(not (POSSESSIVE & YA_EXEMPT), "ولا أصلَ في صنفين معاً")
    idle_class = sorted((POSSESSIVE | YA_EXEMPT | PROPER_NOUNS) - set(bases))
    ok(not idle_class, "ولا مصنَّفٌ ليس أصلاً معلَناً (لا جردَ ميت)"
       + (f" — {idle_class}" if idle_class else ""))
    ...
    #
    # **مأخذُ المدير (١٢ أغسطس ٢٠٢٦)**: «فَوْقَ الشَّجَرَةِ الْعَالِيَةْ» في أربعٍ من
    # ست قصص. والتكرارُ نافعٌ للمبتدئ داخل القصة الواحدة، لكنّ **العبارةَ التي تتنقّل
    # بين القصص** رتابةٌ تُملّ — وهي عيبُ مؤلِّفٍ ضاقت مخيّلتُه لا ضيقُ ميزانية:
    # الرصيدُ ٧٩١ مفردة، وفيه من مواضع المشهد كهفٌ وصخرةٌ وتلٌّ وغصنٌ ونهرٌ وحقل.
    #
    # **والحدُّ مقيسٌ لا مذوَّق**: قصصُ البساتين العشر فيها **صفرُ** عبارةٍ من ثلاث
    # كلماتٍ تتنقّل بين قصتين — فذاك ما يُحتذى. والحدُّ هنا `PHRASE_SPREAD` قصصٍ
    # لعبارة الثلاث، وهو موضعُ أسوأِ ما في المادة المعتمدة اليوم: **يمنع الازدياد**
    # ولا يُعيد فتح ما أُقرّ. ويُطبَع الجردُ كاملاً في كل تشغيل، فتُرى الرتابةُ عدداً.
    spread, spread_of = {}, {}
    for story in stories:
        for text in (p["text"] for p in story["pages"]):
            words = text.split()
            for i in range(len(words) - PHRASE_LEN + 1):
                key = " ".join(words[i:i + PHRASE_LEN])
                spread.setdefault(key, set()).add(story["id"])
    worst = sorted(((len(v), k) for k, v in spread.items() if len(v) > 1), reverse=True)
    over = [(n, k) for n, k in worst if n > PHRASE_SPREAD]
    ok(not over, f"ولا عبارةَ من {PHRASE_LEN} كلماتٍ تتنقّل في أكثر من {PHRASE_SPREAD} قصص"
       + (f" — {[k for _n, k in over]}" if over else ""))
    if worst:
        print(f"      ⤷ أكثرُ العبارات تنقّلاً: "
              + "، ".join(f"«{k}» ({n})" for n, k in worst[:4]))
