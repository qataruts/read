...
console.log('\n— أغلفةُ الرفّ —');
// **ولكل قصةٍ غلافُها بلا استثناء**: أُلحقت العشرُ القديمة ثم قصةُ السورة — والغلافُ
// كسوةٌ لِما هو مكتوب لا محتوىً جديد (البطلُ وجهُ القصة نفسُه، والمزاجُ من المعلَن).
const COVERED = ALL_STORIES;
const SHELF_COVERS = COVERED.map((s) => s.cover);
ok(COVERED.length > 0 && SHELF_COVERS.every(Boolean),
  `لكل قصةٍ غلافٌ معلَن بلا استثناء (${COVERED.length} غلافاً)`);
ok(COVERED.every((s) => s.cover.hero === s.emoji),
  'وبطلُه وجهُ القصة نفسُه — يُقرأ منه ولا يُكتب مرّتين فيفترقا يوماً');
