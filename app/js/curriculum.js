...
export const GROUPS = [
  ...
  },
  {
    ...
    // **الترتيب `ك ع ف ق`** (الحكم ب٥، جلسة وز٣ — تنفيذُ نصّ METHOD §٣ القائم): كان
    // `ع ف ك ق` فتجاورت **ك وق** بلا فاصل، والنصّ يشترط «ك قبل ق بفاصل درسين» —
    // وهما أشبهُ ما في المجموعة أذناً (ولهما زوجُ مواجهةٍ في «ميّز بين» الأولى).
    // وربحت المجموعةُ مع الشرط عزلةً ثانية: **ع** لم تعد تلي **هـ** خاتمةَ الثالثة
    // مباشرةً (مسوّغُ §٣ نفسُه: عزلُ الحلقية الأولى عن هـ وح). والترحيلُ سالم —
    // معرّفُ العقدة `groupId:letter` لا رقمُ الدرس، فنجومُ مَن سبق في مواضعها.
    letters: ['ك', 'ع', 'ف', 'ق'],
    words: [
...
export const CONTRASTS = [
  ...
  },
  {
    ...
    pairs: [
      ...
      // **زوجا الحلق** (الحكم ج٢، جلسة وز٣): كانا غائبين عن المحطتين، وهما من أشدّ
      // ما يلتبس على أذنٍ عربية ناشئة — ع/ح مخرجُهما واحد (وسط الحلق) والفرقُ الجهرُ
      // وحدَه، وغ/خ كذلك (أدنى الحلق). و**موضعُهما الثانية** وإن اكتملت حروفُ ع/ح
      // بالخامسة: الغينُ لا تُدرَّس إلا في السابعة، وجمعُ الحلقيات في مواجهةٍ واحدة
      // أبينُ للفرق من تفريقها على محطتين — والمفكوكية مستوفاة عند g7 يفرضها الفاحص.
      { id: 'ain-hha', letters: ['ع', 'ح'] },
      { id: 'ghayn-kha', letters: ['غ', 'خ'] },
    ],
  },
];
...
export const STORIES = [
  ...
  {
    // **القصة الرابعة** (الحكم ب٦ من المراجعة المستقلة، اعتمد نصَّها المالك ١٥ أغسطس
    // ٢٠٢٦): حروفُ المجموعة السابعة — أصعبُ الحروف نطقاً — كانت تُدرَّس ثم لا تلقى
    // جملةً متصلة حتى البساتين. أبطالُها الثلاثة كلماتُ منهجٍ من مجموعتها، وحروفُها
    // الخمسة كلُّها في المتن (ث ذ ض ظ غ)، والعلاماتُ الستُّ موظفة، وصدقُ الخبر سليم:
    // غرابٌ صاح وضفدعٌ حذِر فقفز — العبرةُ في الحدث لا في دعوى راوٍ.
    id: 'st4',
    after: 'g7',
    title: 'الْغُرَابُ وَالضِّفْدَعْ',
    emoji: '🐦‍⬛',
    sentences: [
      { words: ['ضِفْدَعٌ', 'يَلْعَبُ', 'فِي', 'النَّهْرْ'], emoji: '🐸' },
      { words: ['نَظَرَ', 'الْغُرَابُ', 'مِنَ', 'الشَّجَرْ'], emoji: '🐦‍⬛' },
      { words: ['الثَّعْلَبُ', 'يَمْشِي', 'قُرْبَ', 'الضِّفْدَعْ'], emoji: '🦊' },
      { words: ['صَاحَ', 'الْغُرَابُ', 'فَحَذِرَ', 'الضِّفْدَعْ'], emoji: '😮' },
      { words: ['قَفَزَ', 'الضِّفْدَعُ', 'فِي', 'النَّهْرِ', 'فَنَجَا'], emoji: '💦' },
    ],
    question: { text: 'مَنْ صَاحَ', answer: 'غُرَابْ', distractors: ['ضِفْدَعْ', 'ثَعْلَبْ'] },
  },
];
...
export const QURAN = {
  ...
  letters: {
    ...
    face: 'ء',
    // **قاعدةُ الدرس شُقّت قاعدتين** (أذنُ المالك، ١١ أغسطس ٢٠٢٦): كانت نصّاً واحداً
    // يقتبس الرموز بين قوسين — فنطقها المولّد «ء» «ها» لأنّ **الرمز المفرد لا اسمَ
    // منطوقاً له فيخمّنه**، وبلغت سبع عشرة ثانية. فصار لكل علامةٍ قاعدتُها على
    // بطاقتها **بلا رمزٍ مقتبس**: عشرُ ثوانٍ وقاعدةٌ واحدة في النفَس (قاعدة المدير).
    // والصورُ تبقى معروضةً على الشاشة تُقرأ بالعين — تُعرَض ولا تُنطَق.
    signs: [
      {
    ...
    levels: [
      {
        ...
        items: [
          ...
          // («صدق الصورة»، حكم المدير ٦ أغسطس ٢٠٢٦): تبقى بطاقةً تُنقَر وتُسمع.
          // و`byEar` وسمُ المؤلّف (حكم المدير، ٢٢ أغسطس ٢٠٢٦ — جلسة ع٣): تُسأل في
          // «اقرأ واختر» **بالأذن لا بالصورة** — محطتُها لا تمرينَ مقيساً فيها سواه،
          // فبقيت مهارتُها المفتوحة بلا قياسٍ ١٩٩ يوماً. السؤالُ صوتُها المولَّد
          // القائم ولا صورةَ تُعرَض أصلاً، فصدقُ الصورة سليم.
          { read: 'نُورْ', emoji: '💡', pictured: false, byEar: true },
          { read: 'جَنَّةْ', emoji: '🌳' },
          { read: 'أَرْضْ', emoji: '🌍' },
          { read: 'يَوْمْ', emoji: '📅' },
          { read: 'غَنَمْ', emoji: '🐏' },
  ...
  muqattaat: {
    ...
    face: 'الٓمٓ',
    // السكونُ على أواخر الأسماء الثلاثة مقصود (أذن المالك، ٥ أغسطس ٢٠٢٦): بغيره
    // يصلها المولّدُ فينطقها «أَلِفَ لَامَ مِيمَ»، والمرادُ أسماءٌ موقوفٌ عليها.
    rule: 'في أوائل بعض السور حروف تُقرأ بأسمائها لا بأصواتها: تقول أَلِفْ لَامْ مِيمْ ولا تقول أَلَمْ.',
    items: [
      {
        read: 'الٓمٓ',
        surah: 'البقرة',
  ...
  //
  // **الترتيبُ ترتيبُ الدفعات** (قرار المالك، ١٥ أغسطس ٢٠٢٦، على `REVIEW_METHOD §٢`):
  // كانت اثنتا عشرة سورةً كتلةً واحدة بين بوابة المصحف وبوابة الحديقة — أطولُ امتدادٍ
  // في الرحلة، يقرؤها الطفلُ بأقلّ طلاقةٍ يملكها ثم لا يعود إليها المنهج. فصارت
  // **أربعَ دفعاتٍ ثلاثيةً تتخلّل البساتين** (`SURAHS_PER_STATION` في `progress.js`
  // يقسمها، وترتيبُ هذه القائمة هو ترتيبُها): دفعةٌ ← بستانان ← دفعة… والخاتمةُ
  // قرآنية كما كانت (الدفعة الرابعة آخرَ ما قبل رفّ المكتبة).
  //
  // **والفاتحةُ أولى السور — إعلانُ سببٍ لا سكوت**: هي خلافُ معيار «الطول التصاعدي»
  // الذي اختيرت به الشريحة الثانية (٢٩٥ رمزاً، والكوثرُ ٩٥)، ومسوّغُها **مكانتُها
  // التعبدية**: يصليها الطفل كلَّ يوم فحقُّها أن تكون أولَ ما يقرأ من كلام الله.
  // والإعلانُ عقدُ الشفافية (`METHOD §٥.٦`) — ولا يسري على غيرها: بعدها يصعد الطولُ
  // بالدفعات، وأوّلُ ما يليها الكوثرُ أقصرُ سور المصحف.
  surahs: [
    // ————— الدفعة الأولى: الفاتحة ← الكوثر ← الإخلاص —————
    {
      id: 's1',
      number: 1,
      name: 'الفَاتِحَة',
    ...
    // الكوثرُ **أقصرُ سورةٍ في المصحف** (٩٥ رمزاً، ٣ آيات) وعلاماتُ رسمها عينُ علامات
    // الفاتحة — فأولُ لقاءٍ مستقلٍّ بالرسم العثماني يقع على أصغر نصٍّ ممكن.
    {
      id: 's108',
      number: 108,
      name: 'الكَوْثَر',
    ...
    // ————— الدفعة الثانية: الفلق ← الناس ← العصر —————
    //
    // **الشريحةُ الثانية اختيارٌ لا ترتيب** (حكم المدير على `docs/REVIEW_QURAN_SLICE.md`):
    // اختيرت السورُ الثمانُ بمعيارَي الطول التصاعدي (٩٥ ← ٣٢٨ رمزاً) وأرخصِ رصيدٍ حرفيّ
    // ممكن عند هذا الطول (ثلاثُ علامات رسمٍ وصورةُ همزةٍ لا غير) — **والمعيارُ باقٍ**،
    // وإنما صار الصعودُ **بالدفعات** بعد التوزيع: كلُّ دفعةٍ أطولُ سورةً من سابقتها
    // (١٠٥ ← ١٦٥ ← ٢٠٨ ← ٣٢٨ رمزاً)، والفاتحةُ وحدَها خارجَ المعيار بإعلانِ سببها.
    //
    ...
    //
    // ولا سطرَ من نصّ السور مكتوبٌ هنا بيد: `ayat` تُملأ بـ`sync_quran.py` من المصدر.
    {
      id: 's103',
      number: 103,
      name: 'العَصْر',
    ...
    // ————— الدفعة الثالثة: قريش ← المسد ← (قصةُ الفيل ← كلماتُه ← الفيل) —————
    //
    // وميزانيةُ قصة الفيل تتّسع هنا بلا سطرٍ يُعدَّل: موضعُها صار بعد أربعة بساتين،
    // فرصيدُ الطفل أوسعُ من «كلمات المنهج والمساند» — و«التقديمُ المعلَن» يخفّ تلقائياً.
    {
      id: 's106',
      number: 106,
      name: 'قُرَيْش',
    ...
    // ————— الدفعة الرابعة: الشرح ← الماعون ← القارعة — **الختامُ القرآني** —————
    //
    // أطولُ السور يقرؤها طفلٌ عبَر ستةَ بساتين وسلالمَها ومكتباتِها — فيقرؤها قراءةً
    // لا فكّاً متعثراً، وهو عينُ مقصد «التتويج»: أن يُختَم بالقرآن متقَناً لا أن يُمَرّ به.
    {
      id: 's94',
      number: 94,
      name: 'الشَّرْح',
...
/**
 * الحرفُ مع حركته كما **يُعرَض ويُنطَق** — وهو مفتاحُ ملفّ صوته أيضاً (المفتاح نصُّه).
 *
 * **الألفُ المتحرّكة تُرسم بالهمزة** (الحكم ب٩، جلسة وز٣ — قرار المالك بترجيح
 * المدير): كانت بطاقاتُ درس الألف «اَ اِ اُ»، والألفُ حرفُ مدٍّ لا يقبل حركةً —
 * فالمنطوقُ في البطاقات همزةٌ متحرّكة برسمٍ معتلّ، وكان سيصطدم بدرس الهمزة في
 * المرحلة القرآنية. فصارت **«أَ إِ أُ»**: رسمٌ صحيح من أول يوم **بلا تسميةِ
 * «همزة» بعدُ** — اسمُها وكراسيُّها درسٌ افتتاحيّ هناك، كما يرى الطفلُ السكونَ
 * والمدَّ في «بَابْ» و«بَابَا» قبل درسيهما.
 *
 * **وصفرُ إضافةٍ صوتية**: للنصوص الثلاثة ملفاتُها المولَّدة في `manifest.json`
 * (صُرّفت ٦ أغسطس ٢٠٢٦ بأولوية إصلاحٍ مسموع) — لا توليدَ ولا نسخ.
 */
export function harakaText(letter, mark) {
  const key = HARAKA_BY_MARK[mark];
  if (letter !== 'ا' || !HARAKAT.some((k) => k.key === key)) return letter + mark;
  return (key === 'kasra' ? 'إ' : 'أ') + mark;
}
...
export function quranSpokenTexts() {
  return [
    ...QURAN.letters.signs.map((s) => s.rule),
