...
const ICONS = {
  ...
  // ————— أيقوناتُ بطاقات «المرجع التعريفيّ» (١٨ أغسطس ٢٠٢٦) —————
  //
  // **بطاقةُ واجهةٍ لا بيانات** (`DESIGN §٦`): كانت وجوهُ بطاقات `welcome/` رموزَ
  // Twemoji — وهي لغةُ **بيانات** الطفل لا لغةُ واجهةٍ — فبدت ملصقاتٍ متناثرة لا
  // لغةً واحدة مع التطبيق. **وتُكتب هنا لا في الصفحة** فيبقى للأيقونة مصدرٌ واحد:
  // الصفحاتُ بلا جافاسكربت فتُدرِج المسارَ نصّاً، **وحارسُها يقابله بهذا الجدول
  // حرفاً** (`test_welcome.mjs §١١`) فلا تفترق النسخةُ عن أصلها يوماً.
  // ولا يطلبها اليومَ سطرُ جافاسكربت — فطالبُها صفحةٌ، ويجردها `test_emoji.mjs`
  // من وسم `data-icon` فيها كما يجرد ما تطلبه الشاشات بـ`icon()`.

  // هدفٌ بحلقاته — الإتقانُ وامتحانُ اللحاق: نقطةٌ تُقصَد لا مسافةٌ تُقطَع
  target: '<circle cx="12" cy="12" r="8.6"/><circle cx="12" cy="12" r="4.9"/>'
    + '<circle cx="12" cy="12" r="1.4" fill="currentColor" stroke="none"/>',
  // كفٌّ مبسوطة — وضعُ الدعم: عونٌ يُمَدّ ولا يُنقَص من المنهج
  hand: '<path d="M9.1 11.4V5.3a1.6 1.6 0 0 1 3.2 0v5"/>'
    + '<path d="M12.3 10.4V4.5a1.6 1.6 0 0 1 3.2 0v6.3"/>'
    + '<path d="M15.5 10.9V6.7a1.6 1.6 0 0 1 3.2 0v7.5a6 6 0 0 1-6 6h-1.1'
    + 'a5 5 0 0 1-3.6-1.5L4.7 15a1.7 1.7 0 0 1 2.4-2.4l2 2"/>',
  // درعٌ بعلامة صحّ — فحصٌ يمرّ على المحتوى فيمنع ما لم يُدرَّس
  shield: '<path d="M12 3.2 5.2 5.7v5.5c0 4.2 2.7 7.6 6.8 9.6 4.1-2 6.8-5.4 6.8-9.6V5.7z"/>'
    + '<path d="m8.9 11.8 2.4 2.4 4-4.7"/>',
  // لوحٌ محمول — العملُ دون إنترنت على جهاز الطفل
  device: '<rect x="6.4" y="2.8" width="11.2" height="18.4" rx="2.3"/>'
    + '<path d="M10.6 5.4h2.8"/><circle cx="12" cy="18.2" r="1"/>',
  // مدرسةٌ بسقفها — المعلّمُ والمدرسة
  school: '<path d="M12 3.4 3.7 8.2V20h16.6V8.2z"/><path d="M2.6 20h18.8"/>'
    + '<path d="M10 20v-5.3h4V20"/><path d="M7.5 11.6h2M14.5 11.6h2"/>',
  // خريطةٌ مطويّة — الرحلةُ محطةً محطة
  map: '<path d="m3.6 6.4 5.4-2 6 2 5.4-2v13.2l-5.4 2-6-2-5.4 2z"/>'
    + '<path d="M9 4.4v13.2M15 6.4v13.2"/>',
  // أعمدةُ قياس — لوحةُ المتابعة وما تقرؤه فيها
  chart: '<path d="M4 20h16"/><path d="M6.4 17.6v-5.4"/><path d="M10.8 17.6V8"/>'
    + '<path d="M15.2 17.6v-3.2"/><path d="M19.6 17.6v-7"/>',
  // مكبّرةٌ — مؤشّرٌ يُنظَر فيه (نضجُ القراءة الحرة، وخفوتُ الشكل)
  search: '<circle cx="10.7" cy="10.7" r="6.2"/><path d="m15.3 15.3 4.4 4.4"/>',
  // صورةٌ في إطار — «صدقُ الصورة»: الرمزُ يصدُق كلمتَه
  image: '<rect x="3.6" y="5" width="16.8" height="14" rx="2"/>'
    + '<circle cx="8.9" cy="10.1" r="1.5"/><path d="m4.4 17.4 4.6-4.3 3.4 3.1 3-2.5 4.2 3.7"/>',
  // قلمٌ — حدُّ النطاق: قراءةٌ لا كتابة
  pencil: '<path d="M16.9 3.1a2.8 2.8 0 0 1 4 4L7.4 20.6 2 22l1.4-5.4z"/>'
    + '<path d="m14.9 5.1 4 4"/>',
  // فقاعةُ كلام — الفصحى واللهجة المنطوقة
  speech: '<path d="M20.4 12.3c0 3.8-3.8 6.9-8.4 6.9-1 0-2-.14-2.9-.4L4.2 20.6l1.5-3.6'
    + 'a6.5 6.5 0 0 1-2.1-4.7c0-3.8 3.8-6.9 8.4-6.9s8.4 3.1 8.4 6.9z"/>'
    + '<path d="M8.6 10.2h6.8M8.6 13.4h4.6"/>',
  // مفتاحٌ — البوّابةُ تقيس ولا تدرّس، وما فُتح بها
  key: '<circle cx="15.6" cy="8.4" r="4.2"/><path d="M12.6 11.4 4.3 19.7"/>'
    + '<path d="m5.7 16.3 2.4 2.4"/>',
  // لاقطُ صوت — «اقرأ لي»: صوتُ الطفل لا يغادر جهازه
  mic: '<rect x="9.2" y="2.8" width="5.6" height="10.6" rx="2.8"/>'
    + '<path d="M5.8 11.3a6.2 6.2 0 0 0 12.4 0"/><path d="M12 17.5v3.3"/>'
    + '<path d="M8.9 20.8h6.2"/>',
  // ملفٌّ مكتوب — النسخةُ الاحتياطية من تقدّم الطفل
  file: '<path d="M13.6 3.4H7.4a2 2 0 0 0-2 2v13.2a2 2 0 0 0 2 2h9.2a2 2 0 0 0 2-2V8.4z"/>'
    + '<path d="M13.6 3.4v5h5"/><path d="M8.8 13h6.4M8.8 16.3h4.6"/>',
  // رأسٌ آليّ — أندرويد في خطوات التثبيت (نظامٌ لا كائن)
  robot: '<rect x="4.6" y="8.3" width="14.8" height="11.3" rx="3"/>'
    + '<path d="M12 8.3V5.2"/><circle cx="12" cy="3.8" r="1.2"/>'
    + '<circle cx="9.3" cy="13" r="1" fill="currentColor" stroke="none"/>'
    + '<circle cx="14.7" cy="13" r="1" fill="currentColor" stroke="none"/>'
    + '<path d="M9.9 16.5h4.2"/>',
  // حاسوبٌ محمول — كروم وإيدج في خطوات التثبيت
  laptop: '<rect x="4.6" y="4.8" width="14.8" height="10.2" rx="1.8"/>'
    + '<path d="M2.4 18.6h19.2l-1.7-3.6H4.1z"/>',
