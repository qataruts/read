...
export function renderLibraryStory(storyId) {
  ...
  /* **ومعرّفُ العقدة يتبع محورَ موضعها** (بلاغ المالك، ١٣ أغسطس ٢٠٢٦: «البرنامج توقّف
     ولا يفتح الحلقة التالية»):

     للقصة ثلاثةُ محاور موضعٍ لا محوران — `garden` لمكتبة البستان، و`shelf` لرفّ
     القراءة الطويلة، و**`surah` للمرحلة القرآنية**. وكانت الشاشةُ تعرف اثنين فقط،
     فقصةُ السورة تكتب نجمتَها تحت `library:` بينما الرحلةُ تنتظرها تحت `prophet:`
     (`progress.js` §عقد القرآنية). فالنجمةُ تُحفظ في مكانٍ لا يُقرأ، **والجبهةُ
     تتجمّد عند القصة فلا يُفتَح بعدها شيءٌ أبداً** — وهو ما وقع لجهاز الطفلة عند
     «سُوَرٌ قصار ٣»، أولُ عقدها قصةُ الفيل.

     والمعرّفُ الآن مشتقٌّ من المحور نفسِه — فقصةٌ جديدة بمحورٍ ثالث لا تحتاج سطراً. */
  const kind = shelf ? 'shelf' : (story.surah ? 'prophet' : 'library');
  return readingScreen({
    nodeId: `${kind}:${story.id}`,
    title: story.title,
    emoji: story.emoji,
