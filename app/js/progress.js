...
/**
 * **رحلةٌ بكر**: لا نجمةَ كُسبت ولا محاولةَ سُجّلت في ليتنر — أوّلُ تشغيلٍ حقاً
 * (بطاقةُ أول تشغيل في الخريطة، الجلسة د٣).
 *
 * **وشاهدان لا واحد**: النجومُ وحدَها تكذب على طفلٍ لعب ولم يُتمّ عقدةً (له مهاراتٌ
 * في ليتنر بلا نجمة)، والمهاراتُ وحدَها تكذب على مَن فتح له وليُّه الطريقَ بيده
 * (`unlockUpTo` — نجومٌ بلا محاولة). فأيُّهما وُجد فالرحلةُ قد مُسَّت.
 */
export function isPristine() {
  return totalStars() === 0 && Object.keys(state.skills).length === 0;
}
