...
function renderMap() {
  ...
  // **بابُ التعريف** (أمر المالك، ١٢ أغسطس ٢٠٢٦): من فتح التطبيق وأراد أن يعرف ما
  // هو — معلّمٌ أو وليّ أمرٍ بلغه الرابطُ رأساً — يجد بابَه في ذيل الخريطة لا في
  // صدرها: سطرٌ هادئ بعد المحطات كلِّها، فلا يزاحم درسَ الطفل ولا يشدّ إصبعه.
  // **وهو انتقالُ صفحةٍ لا تنقّلُ تطبيق** (`welcome/` خارج التطبيق وخارج قشرته)،
  // ولذلك **لا يعمل دون إنترنت** — وهو مقصود: صفحةُ عرضٍ للراشد لا تُثقل جهاز طفلٍ
  // بميغاباياتها، ومن يفتحها يفتحها متّصلاً.
...
/* **شريطُ المعاينة ظاهرٌ لا خفيّ** (أمر المالك، ١٣ أغسطس ٢٠٢٦): مَن يفتح التطبيق
   بـ`?preview=1` يرى الرحلةَ كلَّها مفتوحة — فلو لم يُعلَن ذلك لظنّ أنّ هذا ما يراه
   الطفل، **والقفلُ التسلسليّ جوهرُ المنهج**. فالشريطُ يقول ما يجري ويقول إنّ شيئاً
   لا يُحفَظ، ومنه بابٌ إلى التجربة الحقيقية. */
if (progress.PREVIEW) {
